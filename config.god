...
["panda_coins"].each do |app_name|
  ...
  God.watch do |w|
    ...
    w.group = app_name
    w.start = "cd #{app_root}/current/lib/job && RAILS_ENV=production bundle exec clockworkd -c clock_4.rb start --log -d #{app_root}/shared/log"
    w.restart = "cd #{app_root}/current/lib/job && RAILS_ENV=production bundle exec clockworkd -c clock_4.rb restart --log -d #{app_root}/shared/log"
